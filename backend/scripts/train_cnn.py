#!/usr/bin/env python3
"""Offline 1D-CNN trainer for Compare Models artifacts.

CNN is trained offline; results ship as JSON artifacts in
``app/models/artifacts/``. The Render / production runtime does **not**
install torch and does **not** run CNN inference.

Reproduce (from ``backend/``, with requirements-dev.txt installed)::

    PYTHONPATH=. python scripts/train_cnn.py \\
      --ticker SPY --start-date 2020-01-01 --split-date 2022-01-01 \\
      --seq-len 20 --epochs 40 \\
      --out app/models/artifacts/cnn_SPY.json
...
# Allow ``python scripts/train_cnn.py`` from backend/ without requiring PYTHONPATH=.
...
def _require_torch():
    try:
        ...
        raise SystemExit(
            "torch is required for offline CNN training. "
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Offline-train a small 1D CNN for next-day direction and write a "
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Artifact JSON path (default: app/models/artifacts/cnn_<TICKER>.json)",
    )
    parser.add_argument(
        "--weights-out",
        default=None,
...
def _make_cnn(nn: Any, *, n_features: int, channels: int) -> Any:
    class SmallCNN(nn.Module):
        ...
        def forward(self, x: Any) -> Any:
            # (B, seq_len, F) → (B, F, seq_len) for Conv1d
            x = x.transpose(1, 2)
...
def _train_cnn(
    *,
    ...
    model = _make_cnn(nn, n_features=n_features, channels=channels).to(device)
...
def _normalized_equity_rows(backtest_df: pd.DataFrame, label: str = "CNN") -> list[dict[str, Any]]:
...
def train_and_write_artifact(args: argparse.Namespace) -> Path:
    ...
    if args.channels < 8 or args.channels > 64:
        raise SystemExit("--channels should be in [8, 64] for this small model")
    ...
    out_path = Path(
        args.out
        if args.out
        else DEFAULT_ARTIFACT_DIR / f"cnn_{ticker}.json"
    )
    weights_path = Path(
        args.weights_out
        if args.weights_out
    ...
    backtest_df = apply_position_and_returns(
        ...
        buy_reason="CNN predicts next-day up",
        sell_reason="CNN predicts next-day down",
    )
    metrics = calculate_backtest_metrics(backtest_df)
    equity_curve = _normalized_equity_rows(backtest_df, label="CNN")
    ...
    artifact: dict[str, Any] = {
        ...
        "note": (
            ...
            "Reproduce via scripts/train_cnn.py"
        ),
    }
    ...
    try:
        ...
        print(f"Warning: could not write weights ({exc})")
